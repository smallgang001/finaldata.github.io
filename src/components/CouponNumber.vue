    ...
    <table class="table">
      <thead>
        <tr>
          ...
          <th scope="col-7">名稱</th>
          <th scope="col-1">打幾折</th>
          <th scope="col-1">截止日期</th>
          <th scope="col-1">折扣碼</th>
...
<script>
import TanTuo from "../components/CouponTantuo.vue";
...
export default {
  ...
  methods: {
    aSSS(x = 1) {
      this.Load = true;
      const Apiallpage = `${process.env.VUE_APP_API}api/${process.env.VUE_APP_PATH}/admin/coupons?path=${x}`;
      this.axios.get(Apiallpage).then((e) => {
        this.Load = false;
        if (e.data.success) {
          this.pages = Object.values(e.data.coupons);
          this.products = this.pages.filter((e) => {
            return e.category == "優惠券";
          });
    ...
    del(a) {
      ...
      const ApIloojim = `${process.env.VUE_APP_API}api/${process.env.VUE_APP_PATH}/admin/coupon/${this.$refs.a.tt.id}`;
